...
public class SentDataActivity extends AppCompatActivity {
    ...
    private void loadSentHistory() {
        ...
        db.collection("sharedReports")
                ...
                .whereEqualTo("deletedBySender", false)
                .get()
                .addOnSuccessListener(sharedReportSnapshots -> {
                    reports.clear();
                    if (sharedReportSnapshots.isEmpty()) {
                        Toast.makeText(this, "No sent reports found", Toast.LENGTH_SHORT).show();
                    } else {
                        for (QueryDocumentSnapshot document : sharedReportSnapshots) {
                            String reportId = document.getString("reportId");
                            String recipientName = document.getString("recipientName");
                            boolean isReadSender = Boolean.TRUE.equals(document.getBoolean("isReadSender"));
                                    ...
                                    .addOnSuccessListener(reportDoc -> {
                                        if (reportDoc.exists()) {
                                            ...
                                            if (report != null) {
                                                ...
                                                report.setSenderName(currentUserId);
                                                report.setReceiverName(recipientName);
                                                report.setReadSender(isReadSender);
                                            ...
                                            Log.w("SentDataActivity", "Report document does not exist: " + reportId);
                                        }
                                    })
                                    .addOnFailureListener(e -> {
                                        Log.e("SentDataActivity", "Error getting report document: ", e);
                                    });
                        }
                    }
                ...
                .addOnFailureListener(e -> {
                    ...
                    Log.e("SentDataActivity", "Error fetching sent reports: ", e);
                });
    }
        ...
        db.collection("sharedReports")
                ...
                .addOnSuccessListener(querySnapshot -> {
                    if (!querySnapshot.isEmpty()) {
                        ...
                        document.getReference().update("deletedBySender", true)
                                .addOnSuccessListener(aVoid -> {
                                    checkAndDeleteReport(document); // Cek dan hapus jika perlu
                                    reports.remove(report);
                                    adapter.notifyDataSetChanged();
                                    progressBar.setVisibility(View.GONE);
                                    Toast.makeText(this, "Report removed from sent list", Toast.LENGTH_SHORT).show();
                                })
                                .addOnFailureListener(e -> {
                                    progressBar.setVisibility(View.GONE);
    ...
    private void checkAndDeleteReport(QueryDocumentSnapshot document) {
        boolean deletedByRecipient = document.getBoolean("deletedByRecipient") != null && document.getBoolean("deletedByRecipient");
        ...
        db.collection("sharedReports")
                ...
                .addOnSuccessListener(queryDocumentSnapshots -> {
                    for (QueryDocumentSnapshot document : queryDocumentSnapshots) {
                        document.getReference().update("isReadSender", true)
                                .addOnSuccessListener(aVoid -> {
